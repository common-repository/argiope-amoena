...
if ( !class_exists('Argiope_WP_Wrapper'))
	require(dirname(__FILE__).'/class-argiope-wp-wrapper.php');

class Argiope_Validator {
	...
	function __construct( $method = 'POST' ) {
		...
		$this->errors_init();
		$this->rc = new ReflectionClass("Argiope_Validator");
	}
			...
			if ( !is_array($val) ) {
				foreach ( $rules as $rule ) {
					if ( isset($rule['func']) && is_callable($rule['func']) ) {
						...
						if ( Argiope_WP_Wrapper::is_wp_error($val) ) {
							$err = $val;
							return $val;
						}
					}
	...
	private function set_error( $field, $message = '' ) {
		if ( !is_array($message) ) {
			$message = 
				Argiope_WP_Wrapper::is_wp_error($message)
				? Argiope_WP_Wrapper::get_error_message($field, $message)
				: '';
		} else {
			$err = array();
			foreach ( $message as $key => $val ) {
				if ( Argiope_WP_Wrapper::is_wp_error($val) ) {
					$err[$key] = Argiope_WP_Wrapper::get_error_message($field, $val);
				}
			}
	...
	private function esc_html( $val ) {
		return Argiope_WP_Wrapper::esc_html( $val );
	}

	private function required( $val, $field = '' ) {
		if ( empty($val) ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field is required.', $field), $val );
		}
		return $val;
	}
	...
	private function min_length( $val, $field = '', $min_length = false ) {
		...
		if ( strlen($val) < $min_length ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field must be at least %s characters in length.', $field, $min_length), $val );
		}
		return $val;
	}
	...
	private function max_length( $val, $field = '', $max_length = false ) {
		if ( !is_numeric($max_length) )
			return $val;
		if ( strlen($val) > $min_length ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field must be at most %s characters in length.', $field, $max_length), $val );
		}
		return $val;
	}
	...
	private function url( $val, $field = '' ) {
		...
		if ( !preg_match($regex, $val) ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field is invalid.', $field), $val_org );
		}
		return $val;
	}
	...
	private function email( $val, $field = '' ) {
		...
		if ( !($val = Argiope_WP_Wrapper::is_email($val)) ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field is invalid.', $field), $val_org );
		}
		return $val;
	}
	...
	private function match( $val, $field, $regex ) {
		if ( !preg_match($regex, $val) ) {
			return Argiope_WP_Wrapper::wp_error( $field, sprintf('The "%s" field is invalid.', $field), $val );
		}
		return $val;
	}
